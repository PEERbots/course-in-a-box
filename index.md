...
<div class="container py-4">
  ...
  <div class="row">
    ...
    <div class="col-md-6 mb-4">
      <div class="card h-100 shadow-sm border-0 course-card {% if course.status == 'coming_soon' %}opacity-50{% endif %}">
        <div class="card-image-container">
          {% if course.image %}
            <img src="{{ site.baseurl }}/{{ course.image }}" class="card-img-top" alt="{{ course.title }}" style="{% if course.status == 'coming_soon' %}filter: grayscale(80%);{% endif %}">
          {% endif %}
        ...
        <div class="card-body d-flex flex-column">
          {% if course.status == "published" %}
            ...
            <h3 class="card-title"><a href="{{ start_url | strip }}" class="text-dark text-decoration-none">{{ course.title }}</a></h3>
          {% else %}
          ...
          <div class="mt-auto">
            ...
            {% else %}
              <button class="btn btn-secondary btn-lg btn-block" disabled>Coming Soon!</button>
            {% endif %}
          </div>
        </div>
      </div>
